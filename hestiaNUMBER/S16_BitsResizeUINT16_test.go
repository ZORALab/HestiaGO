...
func test_cases_S16_BitsResizeUINT16() []*hestiaTESTING.Scenario {
	return []*hestiaTESTING.Scenario{
		{
			Description: `
test hestiaNUMBER/S16_BitsResizeUINT16 is able to process 16-bits value to
signed 0-bits value.
`,
			Switches: []string{
				cond_TO_BITS_0,
...
`,
			...
			Description: `
test hestiaNUMBER/S16_BitsResizeUINT16 is able to process nil input to
...
func Test_S16_BitsResizeUINT16(t *testing.T) {
	scenarios := test_cases_S16_BitsResizeUINT16()

	for i, s := range scenarios {
		s.ID = uint64(i)
		s.Name = "hestiaNUMBER/S16_BitsResizeUINT16 API"
		...
		if !hestiaTESTING.HasCondition(s, cond_NIL_INPUT) {
			err = S16_BitsResizeUINT16(&subject, size, sign)
		} else {
			err = S16_BitsResizeUINT16(nil, size, sign)
		}
		hestiaTESTING.Log(s, hestiaTESTING.Format("Got Error	: %d", err))

		// assert
		hestiaTESTING.Conclude(s, hestiaTESTING.VERDICT_PASS)
		if !assert_S16_BitsResizeUINT16_output(s, subject) {
			hestiaTESTING.Conclude(s, hestiaTESTING.VERDICT_FAIL)
			t.Fail()
		}

		if !assert_S16_BitsResizeUINT16_error(s, err) {
			hestiaTESTING.Conclude(s, hestiaTESTING.VERDICT_FAIL)
			t.Fail()
		}
...
func assert_S16_BitsResizeUINT16_error(s *hestiaTESTING.Scenario, err hestiaERROR.Error) bool {
	switch {
...
func assert_S16_BitsResizeUINT16_output(s *hestiaTESTING.Scenario, output uint16) bool {
