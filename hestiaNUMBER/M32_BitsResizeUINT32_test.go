...
func test_cases_M32_BitsResizeUINT32() []*hestiaTESTING.Scenario {
	return []*hestiaTESTING.Scenario{
		{
			Description: `
test hestiaNUMBER/M32_BitsResizeUINT32 is able to process 32-bits value to
signed 0-bits value.
`,
			Switches: []string{
				cond_TO_BITS_0,
...
`,
			...
			Description: `
test hestiaNUMBER/M32_BitsResizeUINT32 is able to process nil input to
...
func Test_M32_BitsResizeUINT32(t *testing.T) {
	scenarios := test_cases_M32_BitsResizeUINT32()

	for i, s := range scenarios {
		s.ID = uint64(i)
		s.Name = "hestiaNUMBER/M32_BitsResizeUINT32 API"
		...
		if !hestiaTESTING.HasCondition(s, cond_NIL_INPUT) {
			err = M32_BitsResizeUINT32(&subject, size, sign)
		} else {
			err = M32_BitsResizeUINT32(nil, size, sign)
		}
		hestiaTESTING.Log(s,
		...
		if !assert_M32_BitsResizeUINT32_output(s, subject) {
			hestiaTESTING.Conclude(s, hestiaTESTING.VERDICT_FAIL)
			t.Fail()
		}

		if !assert_M32_BitsResizeUINT32_error(s, err) {
			hestiaTESTING.Conclude(s, hestiaTESTING.VERDICT_FAIL)
			t.Fail()
		}
...
func assert_M32_BitsResizeUINT32_error(s *hestiaTESTING.Scenario, err hestiaERROR.Error) bool {
	switch {
...
func assert_M32_BitsResizeUINT32_output(s *hestiaTESTING.Scenario, output uint32) bool {
