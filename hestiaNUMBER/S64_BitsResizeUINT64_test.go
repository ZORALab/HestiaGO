...
func test_cases_S64_BitsResizeUINT64() []*hestiaTESTING.Scenario {
	return []*hestiaTESTING.Scenario{
		{
			Description: `
test hestiaNUMBER/S64_BitsResizeUINT64 is able to process 64-bits value to
signed 0-bits value.
`,
			Switches: []string{
				cond_TO_BITS_0,
...
`,
			...
			Description: `
test hestiaNUMBER/S64_BitsResizeUINT64 is able to process nil input to
...
func Test_S64_BitsResizeUINT64(t *testing.T) {
	scenarios := test_cases_S64_BitsResizeUINT64()

	for i, s := range scenarios {
		s.ID = uint64(i)
		s.Name = "hestiaNUMBER/S64_BitsResizeUINT64 API"
		...
		if !hestiaTESTING.HasCondition(s, cond_NIL_INPUT) {
			err = S64_BitsResizeUINT64(&subject, size, sign)
		} else {
			err = S64_BitsResizeUINT64(nil, size, sign)
		}
		hestiaTESTING.Log(s,
		...
		if !assert_S64_BitsResizeUINT64_output(s, subject) {
			hestiaTESTING.Conclude(s, hestiaTESTING.VERDICT_FAIL)
			t.Fail()
		}

		if !assert_S64_BitsResizeUINT64_error(s, err) {
			hestiaTESTING.Conclude(s, hestiaTESTING.VERDICT_FAIL)
			t.Fail()
		}
...
func assert_S64_BitsResizeUINT64_error(s *hestiaTESTING.Scenario, err hestiaERROR.Error) bool {
	switch {
...
func assert_S64_BitsResizeUINT64_output(s *hestiaTESTING.Scenario, output uint64) bool {
