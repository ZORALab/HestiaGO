...
func test_cases_MN_Template_Render() []*hestiaTESTING.Scenario {
	return []*hestiaTESTING.Scenario{
		{
			Description: `
test hestiaSTRING/MN_Template_Render is able to process with proper engine and proper writer.
`,
			Switches: []string{
				cond_ENGINE_PROPER,
				cond_WRITER_PROPER,
			...
			Description: `
test hestiaSTRING/MN_Template_Render is able to process with nil engine and proper writer.
`,
			Switches: []string{
				cond_ENGINE_NIL,
				cond_WRITER_PROPER,
			...
			Description: `
test hestiaSTRING/MN_Template_Render is able to process with proper engine and nil writer.
`,
			Switches: []string{
				cond_ENGINE_PROPER,
				cond_WRITER_NIL,
			...
			Description: `
test hestiaSTRING/MN_Template_Render is able to process with nil engine and nil writer.
`,
			Switches: []string{
				cond_ENGINE_NIL,
				cond_WRITER_NIL,
...
func Test_MN_Template_Render(t *testing.T) {
	scenarios := test_cases_MN_Template_Render()

	for i, s := range scenarios {
		s.ID = uint64(i)
		s.Name = "hestiaSTRING/MN_Template_Render API"
		...
		// test
		engine := MN_Template_New("test sample")
		engine.Data = map[string]string{
			"Subject": subject,
			"Target":  target,
		}

		err := MN_Template_ParseString(engine, input)
		...
		_panick := hestiaTESTING.Exec(func() any {
			err = MN_Template_Render(builder, engine)
		...
		if !assert_MN_Template_Render(panick) {
			hestiaTESTING.Conclude(s, hestiaTESTING.VERDICT_FAIL)
			t.Fail()
		}

		if !assert_MN_Template_Render_output(s, expect, output) {
			hestiaTESTING.Conclude(s, hestiaTESTING.VERDICT_FAIL)
			t.Fail()
		}

		if !assert_MN_Template_Render_error(s, err) {
			hestiaTESTING.Conclude(s, hestiaTESTING.VERDICT_FAIL)
			t.Fail()
		}
...
func assert_MN_Template_Render_output(s *hestiaTESTING.Scenario, expect string, output string) bool {
	switch {
...
func assert_MN_Template_Render_error(s *hestiaTESTING.Scenario, err hestiaERROR.Error) bool {
	switch {
	case hestiaTESTING.HasCondition(s, cond_ENGINE_NIL):
...
func assert_MN_Template_Render(panick string) bool {
	return panick == ""
}
